...
describe('safe-language enforcement', () => {
  describe('findUnsafeLanguage', () => {
    ...
    it('allows status-based, behaviour-describing language', () => {
      ...
      expect(
        findUnsafeLanguage('High-risk indicator associated with a crypto investment scam campaign.'),
      ).toBeNull();
      expect(findUnsafeLanguage('Verified scam intelligence — do not enter personal details.')).toBeNull();
  ...
  describe('assertPublicSafeLanguage', () => {
    it('throws a BadRequestException on unsafe text', () => {
      expect(() => assertPublicSafeLanguage('This person is a scammer.', 'publicSafeSummary')).toThrow(
        BadRequestException,
      );
    });

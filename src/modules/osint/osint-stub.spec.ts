...
describe('OSINT stub', () => {
  ...
  it('produces DOMAIN-specific fields and surfaces risk hints', () => {
    const r = stubOsintEnrich({
      indicatorType: 'DOMAIN',
      normalizedIndicator: 'fresh.example.com',
    });
  ...
  it('extracts the email domain for EMAIL indicators', () => {
    const r = stubOsintEnrich({
      indicatorType: 'EMAIL',
      normalizedIndicator: 'attacker@evil.test',
    });
  ...
  it('classifies the chain for CRYPTO_WALLET indicators', () => {
    expect(
      stubOsintEnrich({ indicatorType: 'CRYPTO_WALLET', normalizedIndicator: '0xabc' }).data.chain,
    ).toBe('EVM');
    expect(
      stubOsintEnrich({ indicatorType: 'CRYPTO_WALLET', normalizedIndicator: 'bc1xyz' }).data.chain,
    ).toBe('BTC');
  });

  it('infers country code for E.164 phone numbers', () => {
    expect(
      stubOsintEnrich({ indicatorType: 'PHONE', normalizedIndicator: '+15551234567' }).data.country,
    ).toBe('US');
    expect(
      stubOsintEnrich({ indicatorType: 'PHONE', normalizedIndicator: '+447700900123' }).data
        .country,
    ).toBe('GB');
  });
});
